...
interface PackageWithProvider extends PackageType {
...
export default function EditPackagePage() {
  ...
  const [packageData, setPackageData] = useState<PackageWithProvider | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const packageData: PackageWithProvider = await packageRes.json();

        // Set form fields with package data
        setName(packageData.name);
        setSpeed(packageData.speed);
        setPrice(packageData.price);
        setPurchasePrice(packageData.purchasePrice || 0);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        showNotification('error', data.error || 'Failed to update package');
      }
    } catch (err) {
      showNotification('error', 'An error occurred while updating the package');
      console.error(err);
    } finally {
      setSubmitting(false);
    }
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-500 dark:text-gray-400 mt-1">
              Update internet service package details
            </p>
          </div>
        </div>
      </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500 dark:text-gray-400">
                Modify the specifications and pricing for this package
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="md:col-span-2">
              ...
              <div className="relative">
                ...
                <select
                  ...
                  onChange={(e) => setServiceProviderId(e.target.value || null)}
