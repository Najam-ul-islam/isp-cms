...
import { 
  ...
  Calendar, Clock, AlertCircle, TrendingUp,
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-slate-50 via-white to-blue-50/30">
        ...
        <div className="flex items-center justify-between max-w-7xl mx-auto">
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              ...
              title="Refresh data"
            >
              <RefreshCw className={`w-5 h-5 text-slate-600 group-hover:text-blue-600 transition-colors ${loading ? 'animate-spin' : ''}`} />
            </button>
      ...
      <main className="max-w-7xl mx-auto px-8 py-8 space-y-8">
        ...
        {/* EXPIRATION ALERTS */}
        <Section title="Expiration Alerts" icon={<AlertCircle className="w-5 h-5" />} variant="warning">
          <div className="grid md:grid-cols-3 gap-5">
            <AlertCard
              title="Expiring Today"
              value={stats?.expireToday ?? 0}
              urgency="critical"
              message={stats?.expireToday ? "Immediate action required" : "All clear! 🎉"}
            />
            <AlertCard
              title="Next 3 Days"
        ...
        {/* BILLING METRICS */}
        <Section title="Billing Overview" icon={<DollarSign className="w-5 h-5" />} variant="success">
          <div className="grid md:grid-cols-3 gap-5">
            <FinanceCard
              title="Paid Today"
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200/60 overflow-hidden">
          <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between bg-linear-to-r from-purple-50/50 to-transparent">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="font-semibold text-slate-800">Clients Expiring Soon</h2>
                <p className="text-sm text-slate-500">Proactive retention opportunities</p>
              </div>
            </div>
                ...
                {expiringClients?.length ? (
                  expiringClients.map((client, index) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <button className="px-3 py-1.5 text-sm font-medium text-blue-600 hover:bg-blue-50 rounded-lg transition-colors opacity-0 group-hover:opacity-100">
                          Contact
                        </button>
                      </td>
                    </tr>
                  ...
                  <tr>
                    <td colSpan={6} className="px-6 py-12 text-center">
                      <div className="flex flex-col items-center gap-3 text-slate-400">
                        ...
                        <p className="text-sm">All your clients are up to date! 🎉</p>
                      </div>
                    </td>
                  </tr>
                )}
  ...
  return (
    <section className="space-y-4">
      <div className="flex items-center gap-2">
        <div className={`p-2 rounded-lg bg-linear-to-br ${variants[variant]} shadow-lg`}>
...
function StatCard({ 
  ...
  const colors = {
    blue: { bg: 'from-blue-50 to-blue-100/50', icon: 'bg-blue-500', text: 'text-blue-600', border: 'border-blue-200' },
    emerald: { bg: 'from-emerald-50 to-emerald-100/50', icon: 'bg-emerald-500', text: 'text-emerald-600', border: 'border-emerald-200' },
    rose: { bg: 'from-rose-50 to-rose-100/50', icon: 'bg-rose-500', text: 'text-rose-600', border: 'border-rose-200' },
    amber: { bg: 'from-amber-50 to-amber-100/50', icon: 'bg-amber-500', text: 'text-amber-600', border: 'border-amber-200' },
    purple: { bg: 'from-purple-50 to-purple-100/50', icon: 'bg-purple-500', text: 'text-purple-600', border: 'border-purple-200' },
  }

  const c = colors[color]

  return (
    <div className={`relative bg-white rounded-2xl p-5 shadow-sm border ${c.border} hover:shadow-md transition-all duration-300 group overflow-hidden`}>
      {/* Decorative gradient blob */}
      <div className={`absolute -top-10 -right-10 w-32 h-32 bg-linear-to-br ${c.bg} rounded-full blur-2xl opacity-60 group-hover:opacity-80 transition-opacity`} />
      
      <div className="relative flex items-start justify-between">
        <div className="space-y-1">
          <p className="text-sm font-medium text-slate-500">{title}</p>
          <p className="text-3xl font-bold text-slate-800 tracking-tight">{value.toLocaleString()}</p>
          
          {trend && (
            <div className={`flex items-center gap-1 text-sm ${trend.positive ? 'text-emerald-600' : 'text-rose-600'}`}>
        ...
        <div className={`p-3 rounded-xl bg-linear-to-br ${c.icon} shadow-lg shadow-${color}-500/25 text-white`}>
          {icon}
        </div>
      </div>
    </div>
...
function AlertCard({ 
  ...
  const urgencies = {
    critical: { bg: 'from-rose-500 to-pink-500', ring: 'ring-rose-200', text: 'text-rose-600', badge: 'bg-rose-100 text-rose-700' },
    high: { bg: 'from-amber-500 to-orange-500', ring: 'ring-amber-200', text: 'text-amber-600', badge: 'bg-amber-100 text-amber-700' },
    medium: { bg: 'from-blue-500 to-cyan-500', ring: 'ring-blue-200', text: 'text-blue-600', badge: 'bg-blue-100 text-blue-700' },
  }

  const u = urgencies[urgency]

  return (
    <div className={`relative bg-white rounded-2xl p-5 shadow-sm border border-slate-200/60 hover:shadow-md transition-all duration-300 overflow-hidden group`}>
      <div className={`absolute top-0 left-0 right-0 h-1 bg-linear-to-r ${u.bg}`} />
      ...
      <div className="flex items-start justify-between mb-3">
        ...
        {value > 0 && (
          <span className={`px-2.5 py-1 text-xs font-semibold rounded-full ${u.badge}`}>
            {value} clients
          </span>
        )}
      </div>
...
function FinanceCard({ 
  ...
  const types = {
    income: { color: 'text-emerald-600', bg: 'bg-emerald-50', border: 'border-emerald-200' },
    due: { color: 'text-rose-600', bg: 'bg-rose-50', border: 'border-rose-200' },
    upcoming: { color: 'text-amber-600', bg: 'bg-amber-50', border: 'border-amber-200' },
  }

  const t = types[type]

  return (
    <div className={`bg-white rounded-2xl p-5 shadow-sm border border-slate-200/60 hover:shadow-md transition-all duration-300`}>
      ...
      <div className={`mt-3 pt-3 border-t ${t.border}`}>
        <div className="flex items-center gap-2 text-sm">
          <div className={`w-2 h-2 rounded-full ${type === 'income' ? 'bg-emerald-500' : type === 'due' ? 'bg-rose-500' : 'bg-amber-500'}`} />
          <span className={t.color}>{type === 'income' ? 'Received' : type === 'due' ? 'Pending' : 'Forecasted'}</span>
        </div>
      </div>
    </div>
  )
      ...
      <main className="max-w-7xl mx-auto px-8 py-8 space-y-8">
        {[1, 2, 3].map((section) => (
          <div key={section} className="space-y-4">
            ...
            <div className="grid md:grid-cols-3 gap-5">
              {[1, 2, 3].map((card) => (
                <div key={card} className="bg-white rounded-2xl p-5 border border-slate-200">
